...
- Install the autostart entry for Hyprland
...
2. The daemon will start automatically when Hyprland starts
...
**Option B: Hyprland autostart (recommended for permanent setup)**
```bash
# Add to Hyprland autostart
echo 'exec-once = python3 /path/to/open-borderless-daemon' >> ~/.config/hypr/autostart.conf

# Verify it's running
ps aux | grep open-borderless-daemon
```
...
#   exec-once = python3 /path/to/open-borderless-daemon
